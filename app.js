...
const cbName = "_cb" + Math.random().toString(36).slice(2);
...
row.className = "row";
row.innerHTML =   ${m.photoUrl ?<img class="avatar" src="${esc(m.photoUrl)}" alt="사진">:<div class="avatar"></div>}   <div class="row-main">   <div class="row-title">${esc(m.name)} ${m.position ? <span class="badge">${esc(m.position)}</span>` : ""}</div>
...
const row = document.createElement("div");
row.className = "row";
row.innerHTML =    <div class="row-main">   <div class="row-title">${esc(a.title || "")}</div>   <div class="row-sub">${esc(a.date || "")} ${a.author ? " · " + esc(a.author) : ""}</div>   <div class="row-sub" style="white-space:normal;margin-top:8px;">${esc(a.body || "")}</div>   </div>;
...
const row = document.createElement("div");
row.className = "row";
row.innerHTML =   <div class="row-main">   <div class="row-title">   ${esc(a.title || "")}   ${isAnnNew(a) ?<span class="badge-new">NEW</span>` : ""}

</div>  <div class="row-sub">${esc(a.date || "")} ${a.author ? " · " + esc(a.author) : ""}</div>  
  </div>`;  
wrap.appendChild(row);
...
showHint(  ⚠️ 이 브라우저에서는 앱 설치가 불가능합니다.<br><br>   <b>반드시 Chrome 브라우저에서 설치</b>해 주세요.  );
return;
}

// ❌ 크롬이지만 설치 트리거가 안 잡힌 경우
if (!deferredPrompt) {
showHint(  ⚠️ 설치 환경에 문제가 발생했습니다.<br>   관리자에게 문의하세요.  );
return;
}
...
showHint(  <b>아이폰 설치 방법(사파리)</b><br/>   1) 사파리로 이 페이지 열기<br/>   2) 아래 <b>공유(⬆️)</b> 버튼 누르기<br/>   3) <b>홈 화면에 추가</b> 선택<br/>   4) 추가 → 홈화면 아이콘으로 실행  );
});
...
openModal(`
  ...
  <!-- 날짜 제목 : 가운데 정렬 -->    <div style="text-align:center;margin-bottom:12px;">  
